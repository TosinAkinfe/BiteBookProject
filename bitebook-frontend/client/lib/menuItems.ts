const MENU_PATTERNS: Array<{ match: string[]; items: string[] }> = [
  {
...
export function findMatchingMenuItems(
  restaurant: {
    name?: string;
    cuisine?: string;
    venueType?: string;
  ...
  const cuisineHaystack = normalizeText(
    [restaurant.cuisine, restaurant.venueType].filter(Boolean).join(" "),
  );

  if (cuisineHaystack.includes(normalizedQuery)) {
    return menuItems;
  }
